        ...
        if (i == 18) {
            ...
            if (DeviceProperties.isWearable(context)) {
                ...
                if (DeviceProperties.isWearableWithoutPlayStore(context)) {
                    style.addAction(C0356R.C0357drawable.common_full_open_on_phone, resources.getString(C0356R.string.common_open_on_phone), pendingIntent);
                } else {
                    style.setContentIntent(pendingIntent);
                }
            } else {
                style.setSmallIcon(17301642).setTicker(resources.getString(C0356R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setContentIntent(pendingIntent).setContentText(errorNotificationMessage);
            }
            if (PlatformVersion.isAtLeastO()) {
